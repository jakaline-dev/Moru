...
def convert_unet_checkpoint(checkpoint, device=None, dtype=None):
    ...
    converted_unet_checkpoint = convert_ldm_unet_checkpoint(checkpoint, config=unet_config)
    with ctx():
        unet = UNet2DConditionModel(**unet_config)
    if is_accelerate_available():
        for param_name, param in converted_unet_checkpoint.items():
            set_module_tensor_to_device(unet, param_name, device=device, value=param, dtype=dtype)
    else:
...
def convert_vae_checkpoint(checkpoint, device=None, dtype=None):
    ...
    if is_accelerate_available():
        for param_name, param in converted_vae_checkpoint.items():
            set_module_tensor_to_device(vae, param_name, device=device, value=param, dtype=dtype)
    else:
...
def convert_text_encoder_checkpoint(checkpoint, device=None, dtype=None):
    ...
    if is_accelerate_available():
        for param_name, param in text_model_dict.items():
            set_module_tensor_to_device(text_model, param_name, device=device, value=param, dtype=dtype)
    else:
...
def load_sdxl_ckpt(file_path, device="cpu", vae_path=None, dtype=None):
    ...
    # tokenizer
    tokenizer = CLIPTokenizer.from_pretrained(SDXL_CONFIG_PATH / 'tokenizer', local_files_only=True)
    tokenizer_2 = CLIPTokenizer.from_pretrained(SDXL_CONFIG_PATH / 'tokenizer_2', pad_token="!", local_files_only=True)
    # text_encoder
    text_encoder = convert_text_encoder_checkpoint(checkpoint, device=device, dtype=dtype)
    text_encoder_2 = convert_text_encoder_2_checkpoint(checkpoint, device=device, dtype=dtype)
    # scheduler
...
if __name__ == "__main__":
    load_sdxl_ckpt(r"C:/CODE/ComfyUI_windows_portable/ComfyUI/models/checkpoints/sd_xl_base_1.0.safetensors")